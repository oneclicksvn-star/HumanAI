...
const COMMITMENTS = [
  { type: "📅", agent: "Luna", content: "Follow up on Q4 analysis results", due: "in 2 hours" },
  { type: "💚", agent: "Sage", content: "Weekly team wellness check-in", due: "tomorrow 9am" },
  { type: "⏰", agent: "Atlas", content: "PR review deadline for main branch", due: "today 5pm" },
];
...
export default function Dashboard() {
  ...
  const STAT_CARDS = [
    { label: "Active Agents", value: stats?.activeAgents ?? agentList.length, icon: Users, color: "text-indigo-600", bg: "bg-indigo-50 border-indigo-100", badge: "+2 new" },
    { label: "Sessions Today", value: stats?.sessionsToday ?? 0, icon: Activity, color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-100", badge: "active" },
    { label: "Tokens Used", value: stats?.tokensToday ? `${(stats.tokensToday / 1000).toFixed(0)}K` : "0", icon: Zap, color: "text-violet-600", bg: "bg-violet-50 border-violet-100", badge: "budget" },
    { label: "Tasks In Progress", value: stats?.openTasks ?? 0, icon: CheckCircle, color: "text-amber-600", bg: "bg-amber-50 border-amber-100", badge: "open" },
  ];
    ...
    <div className="min-h-screen bg-[#f7f8fc] p-7 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Your AI agents are running strong — {agentList.length} active, all systems nominal</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-white rounded-2xl border border-gray-100 px-4 py-2 shadow-sm text-[11px]">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" /><span className="text-gray-600 font-semibold">All systems healthy</span>
          </div>
          <Link href="/agents">
      ...
      <div className="grid grid-cols-3 gap-5">
        <div className="col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-800">Agent Status</h2>
            <Link href="/agents"><span className="text-xs text-indigo-500 flex items-center gap-1 hover:underline cursor-pointer">View all <ChevronRight size={12} /></span></Link>
          </div>
          {agentList.map(a => (
            <div key={a.id} className="bg-white rounded-2xl border border-gray-100 p-5 flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="text-right">
                <div className="flex items-center gap-1 text-[10px] text-gray-400"><span>Energy</span><p className="font-bold text-sm text-gray-700">{a.energy}%</p></div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-bold text-gray-700">System Pulse</h3>
            ...
            <div className="mt-3 space-y-2">
              {(activity ?? []).slice(0, 4).map((a, i) => (
                <div key={i} className="flex items-start gap-2 text-[11px]">
                  <span>{a.type === "message" ? "💬" : a.type === "tool_call" ? "⚡" : a.type === "memory_store" ? "🧠" : "✅"}</span>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
            <h3 className="text-xs font-bold text-gray-700 mb-3">Commitments</h3>
            {COMMITMENTS.map((c, i) => (
              <div key={i} className="flex items-start gap-2.5 py-2 border-b border-gray-50 last:border-0">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[9px] text-gray-400">Due: {c.due}</p>
                </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-bold text-gray-800">Dream Insights</h2>
          <span className="text-[10px] text-gray-400">— Latest consolidation from Dream Mode</span>
        </div>
            ...
            <div key={i} className="bg-gray-50 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-gray-400">Dream Mode</p>
                </div>
              </div>
